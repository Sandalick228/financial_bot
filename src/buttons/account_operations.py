...
async def select_category_kb():
    categories = await transaction_db.get_all_categories()
    keyboard = InlineKeyboardBuilder()
    for category in categories:
        keyboard.row(InlineKeyboardButton(text=category.name, callback_data=f'select_categories_{category.id}'))
    keyboard.row(InlineKeyboardButton(text='◀️ Назад', callback_data='back_to_main'))
    return keyboard.as_markup()

async def select_subcategory_kb(category_id):
    subcategories = await transaction_db.get_subcategories(category_id)
    keyboard = InlineKeyboardBuilder()
    for subcategory in subcategories:
        keyboard.row(InlineKeyboardButton(text=subcategory.name, callback_data=f'select_subcategories_{subcategory.id}'))
    keyboard.row(InlineKeyboardButton(text='◀️ Назад', callback_data='back_to_main'))
    return keyboard.as_markup()

async def keyboard_get_accounts(tg_id):
    payment_accounts = await user_db.get_accounts(tg_id)
    items = InlineKeyboardBuilder()
    for payment_account in payment_accounts:
        items.row(InlineKeyboardButton(text=payment_account.name, callback_data=f'accounts_get_{payment_account.id}'))
